...
For example if the source=mysql then the child variables can be any of the variables listed in [mysql variables](/guides/mysql) but if the source is oracle then the variables need to be [oracle variables](/guides/oracle).
...
For example if the target=snowflake then the child variables can be any of the variables listed in [snowflake variables](/guides/snowflake).
...
For example the if the target= firebolt you can see both loader and mover variables in [firebolt variables](/guides/firebolt).
