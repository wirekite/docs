...
<Steps>
  ...
  <Step title="Loaders" />
    This is the component that reads the file which are moved by the mover and "loads" them onto the target databases. Loaders work on schema, data and change files. Loaders essentially read the files and transalate them to corresponding database language statements and execute those statements on the database. 
</Steps>
