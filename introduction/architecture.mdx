...
<Steps>
  <Step title="Schema">
Schema essentially refers to the "structure" of the data. It constitutes the definition of tables, the columns, their datatypes, any other properties like are they primary keys, unique keys, foreign keys etc. This is a one time task t
  </Step>

  <Step title="Data">
Data at this stage refers to the initial dump of data. Usually this means a large number of tables which can be large in size (TBs/PBs). Note that this is the 

  </Step>
  <Step title="Change">
Change refers to the constant "changes" that happen to the source database. Typically this means Inserts, Updates and Deletes. Note that this happens infinitely and Wirekite establishes a pipeline 
...
Wirekite essentially has 3 components, each of these components&#x20;
