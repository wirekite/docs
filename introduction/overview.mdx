...
* **Performant**
  ...
  \- We pride ourselves in being extremely fast. Please visit our [benchmarks](https://wirekite.webflow.io/)
...
* **Cross Database**
  ...
  \- Wirekite allows you to move data from Postgres, MySQL and Oracle to multiple cloud databases. For now we are supporting Cloud versions of MySQL and Postgres and Snowflake. We will be adding more Cloud targets in future.

* **Extract, Load**

  \- Wirekite to allows you to extract and load data from source databases. Part of any data migration (whether one time or ongoing) involves taking an initial extract of data and moving it to the target. Wirekite prides itself in moving copious amounts of data (TBs / PBs) in much shorter time frames than other technologies.
...
* **Replication**
  ...
  \- In addition to moving large data, wirekite allows you to keep the data on the target fresh from the source. This helps enormously with doing an application switch over from source to target while affording relatively small downtime.
