...
- **Performant**- We pride ourselves in being extremely fast. Please visit our [extract and load benchmarks](https://benchmarks.wirekite.io/series/benchmarks-extract-and-load) and [replication benchmarks](https://benchmarks.wirekite.io/series/benchmarks-replication).

<Note>
  **Wirekite is 10x-100x faster than its competitors.**
</Note>

- **Cross Database**- Wirekite allows you to move data from Oracle, Postgres, MySQL, MariaDB, and SQL Server to multiple cloud and on-premise databases. For now we are supporting Cloud versions of MySQL, MariaDB, PostgreSQL, Snowflake, and Firebolt. We will be adding more Cloud targets in future.
- **Extract, Load**- Wirekite to allows you to extract and load data from source databases to targets. Part of any data migration - whether one time or ongoing - involves taking an initial extract of data and moving it to the target. Wirekite prides itself in moving copious amounts of data in much shorter time frames than other technologies.

<Card title="Petabyte Scale" icon="truck-moving" href="/introduction/overview">
  Wirekite can migrate tables with **billions** of rows within **minutes**.
</Card>
