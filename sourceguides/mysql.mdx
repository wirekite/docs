...
To configure MySQL or MariaDB as an extractor you must have the following prereqs met.  Below references to MySQL also apply to MariaDB setups.
 ...
 9. When doing an initial data extract make sure that replication is stopped. This is to make sure that the dump is consistent to given mysql coordinate position.
...
11. If you're extracting from a cloud-managed or remote instance that can't dump to a filesystem you can access, be sure to use the loadLocal=true config option below.
...
<ResponseField name="loadLocal" type="boolean" default="false">
  Whether to use the "local" dump option for MySQL, which means the dump will be done to files visible on the client host, no matter where the database instance itself is running.  **loadLocal=true is necessary for cloud-managed MySQL sources.**
</ResponseField>
